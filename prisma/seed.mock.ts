...
async function main() {
  ...
  // Seed Accounts & Users
  const accountsData = [
    { username: 'dev', role: 'DEVELOPER', name: 'Dev User', position: 'Developer', phone: '0800000001' },
    { username: 'admin', role: 'ADMIN', name: 'Admin User', position: 'Admin', phone: '0800000002' },
    { username: 'user1', role: 'USER', name: 'User One', position: 'Pharmacist', phone: '0800000003' },
    { username: 'user2', role: 'USER', name: 'User Two', position: 'Nurse', phone: '0800000004' },
    { username: 'unapproved', role: 'UNAPPROVED', name: 'Unapproved User', position: 'Intern', phone: '0800000005' },
  ];
  ...
  const accounts = await prisma.account.findMany({ where: { organizationId: org.id }, include: { user: true } });
  ...
  const reporterAccounts = accounts.filter(a => a.role !== 'UNAPPROVED');
